...
Select a dataset type and analysis method to extract giraffe torsos and generate key point-based descriptors for reidentification. The giraffe’s coat pattern acts as a unique fingerprint for matching.  

**1. Dataset Selection**  
Reference Catalogue focuses on annotated giraffes used for comparison.  
Query Data focuses on new images matched against the reference dataset.  

**2. Segmentation & Torso Detection**  
A pretrained model isolates the central giraffe.  
A fine-tuned detector extracts the torso.  

**3. Image Descriptors Extraction**  
SIFT extracts key points and descriptors from torso-only images after background removal.  
...
if len(additional_args) != 0:
    ...
    selected_option = st.selectbox("", ['Extract Giraffe Torsos', 
    ...
    if selected_option == 'Extract Giraffe Torsos':
        pycode_name = 'step_1_run_vision_to_crop_torso.py'
        main_run_buttons(pipeline_code_dir, pycode_name, demo_images[1], additional_args)

    elif selected_option == 'Extract Image Descriptors': 
        pycode_name = 'step_2_create_image_discriptors.py'      
